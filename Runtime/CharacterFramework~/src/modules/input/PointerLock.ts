...
/** Wrapper for PointerLock API */
export class PointerLock extends EventDispatcher<any> {
	domElement: HTMLElement;
	rawMouseWhileLocked: boolean;
	...
	constructor(domElement: HTMLElement, rawMouseWhileLocked: boolean) {
		super();

		this.domElement = domElement;
		this.rawMouseWhileLocked = rawMouseWhileLocked;
	...
	async lock() {
		...
		try {
			...
			await this.domElement.requestPointerLock({
				unadjustedMovement: this.rawMouseWhileLocked,
			});
		}
		catch (e) {
			if (debug)
